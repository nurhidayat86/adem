...
def compute_feature(slot_data):
    ...
    feature = [min[0], min[1], min[2], min123, max[0], max[1], max[2], max123, mean[0], mean[1], mean[2], mean123, std[0], std[1], std[2], std123, sad[0], sad[1], sad[2], sad123, corl1, corl2, corl3, corl123, onoff[0], onoff[1], onoff[2], onoff123, range[0], range[1], range[2], range123];
    return feature;
...
# run SVM classifier
svc = svm.SVC(kernel='rbf', C=0.2, gamma=0.0299999);
start_time = time.time();
