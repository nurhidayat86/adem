...
################################################################################################
############################# NILMTK ECO disaggregation ########################################
################################################################################################

def nilmtkECOfunc(train_start, train_end, test_start, test_end, output_period):
    #### configuration ####
    ...
    start = time.time()
    disag_filename = '/home/neo/NILMTK_experimental/eco-b2-kall-co-1w:11-1m.h5'
...
#    print("Ja  all",  Ja_co_all)
    ...
    for i in disag_co_elec.submeters().instance():
        label.append(str(disag_co_elec[i].label()).lower());
    disag_co_elec_submeter_df.columns = label;
    gt_df_sub.columns = label;
